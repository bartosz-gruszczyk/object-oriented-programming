...
// std::shared_ptr<Cargo> Store::findCargoInStore(const std::shared_ptr<Cargo>
// wantedCargo) {
...
Response Store::sell(std::shared_ptr<Cargo>& cargo,
    size_t amount,
    Player* player)
{
    // lack of an amount of cargo in store

    std::cout << "nazwa ładunku: " << cargo->getName()
              << " ilosć ładunku: " << amount
              << " ile miejsc wolnych u zawodnika: "
...
void Store::nextDay()
{
    for (auto cargo : Store::getCargoOfStore()) {
        if (cargo == nullptr) {
            ...
            std::uniform_int_distribution<> in_store_action(less_stuff_in_store,
                more_stuff_in_store);
            std::uniform_int_distribution<> to_add_or_take(adding_taking_min,
                adding_taking_max);
